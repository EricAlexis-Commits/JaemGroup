...
namespace GIMNASIOJAEM
{
    public partial class Form1 : Form
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            ...
            if (tbUserName.Text!="" || tbUserName.Text != null && tbUserPassword.Text!="" || tbUserPassword.Text!=null)
            {
                login.seleccionarUsuario(tbUserName.Text, tbUserPassword.Text);
        ...
        class CMD
        {
            public void seleccionarUsuario(string nombre, string contraseña)
            {
                try
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        Sension.usuarioActual = dt.Rows[0]["Nombre_Usuario"].ToString();
                        Sension.permisoUsuario = dt.Rows[0]["Tipo_Usuario"].ToString();
